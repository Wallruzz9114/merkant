...
public class BarcodeProductAttribute : ValidationAttribute
{
    ...
    private bool Ean13IsValid(string codigoDeBarras)
    {
        ...
        string ean13 = codigoDeBarras;

        IEnumerable<int> pares = ean13.Take(codigoDeBarras.Length - 1).Where((c, i) => i % 2 == 0).Select(c => int.Parse(c.ToString()));
        IEnumerable<int> impares = ean13.Take(codigoDeBarras.Length - 1).Where((c, i) => i % 2 != 0).Select(c => int.Parse(c.ToString()));
